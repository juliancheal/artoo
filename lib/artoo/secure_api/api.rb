require 'reel'
require 'artoo/secure_api/activation'
require 'artoo/secure_api/route_helpers'
require 'artoo/secure_api/device_event_client'

module Artoo
  module SecureApi
    # Artoo Secure API Server provides an interface to communicate with
    ...
    class Server < Reel::Server::HTTP
      ...
      def initialize(host = "127.0.0.1", port = 3000,
                      auth_host = "127.0.0.1", auth_port = 5000,
                      application_id="", secret="")
                      
        @auth_host = auth_host
        @auth_port = auth_port
        
        @activation = Activation.new(host, port, auth_host,
                                auth_port, application_id, secret)
      ...
      any 'oauth2/callback' do
        @activation.get_token(@params)
      end
      ...
      # @return [JSON] command
      any '/api/robots/:robotid/commands/:commandid/?:access_token' do
        if is_authenticated?
          validate_params!
          result = @robot.command(@params['commandid'], *command_params)
          return MultiJson.dump({'result' => result})
        else
          return MultiJson.dump({'error' => 'not_authorized'})
        end
      end
      ...
      def is_authenticated?
        @activation.validate_token(@params['access_token'])
      end
